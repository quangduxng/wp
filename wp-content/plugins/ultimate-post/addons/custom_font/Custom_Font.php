...
class Custom_Font {
    ...
    function set_data($arr = [], $font_name='') { ?>
        <div class="ultp-custom-font-container ultp-custom-font<?php echo empty($arr) ? '-copy' : ''; ?>">
            <div class="ultp-custom-font-heading">
                <div>
                    <label class="font-label"><?php _e('Weight:  ', 'ultimate-post'); ?> <span class="ultp-custom-font-weight"> <?php _e(isset($arr['weight']) ? $arr['weight'] : '', 'ultimate-post'); ?> </span></label>
                    <select name="weight[]">
                        <?php $weight = isset($arr['weight']) ? $arr['weight'] : ''; ?>
                        <option <?php selected( $weight, 'normal' ); ?> value="normal"><?php _e('Normal', 'ultimate-post'); ?></option>
                        <option <?php selected( $weight, '100' ); ?> value="100"><?php _e('100', 'ultimate-post'); ?></option>
                        <option <?php selected( $weight, '200' ); ?> value="200"><?php _e('200', 'ultimate-post'); ?></option>
                        <option <?php selected( $weight, '300' ); ?> value="300"><?php _e('300', 'ultimate-post'); ?></option>
                        <option <?php selected( $weight, '400' ); ?> value="400"><?php _e('400', 'ultimate-post'); ?></option>
                        <option <?php selected( $weight, '500' ); ?> value="500"><?php _e('500', 'ultimate-post'); ?></option>
                        <option <?php selected( $weight, '600' ); ?> value="600"><?php _e('600', 'ultimate-post'); ?></option>
                        <option <?php selected( $weight, '700' ); ?> value="700"><?php _e('700', 'ultimate-post'); ?></option>
                        <option <?php selected( $weight, '800' ); ?> value="800"><?php _e('800', 'ultimate-post'); ?></option>
                        <option <?php selected( $weight, '900' ); ?> value="900"><?php _e('900', 'ultimate-post'); ?></option>
                    </select>
                </div>
                <?php
                    $styles = '';
                ...
                ?>
                <span <?php echo $styles; ?> class="ultp-custom-font-preview"><?php _e('The quick brown fox jumps over the lazy dog', 'ultimate-post'); ?></span>
                <div class="ultp-custom-font-actions">
                    <span class="ultp-custom-font-edit"><span class="dashicons dashicons-edit"></span><?php _e('Edit', 'ultimate-post'); ?></span>
                    <span class="ultp-custom-font-close"><span class="dashicons dashicons-no-alt"></span><?php _e('Close', 'ultimate-post'); ?></span>
                    <span class="ultp-custom-font-delete"><span class="dashicons dashicons-trash"></span><?php _e('Delete', 'ultimate-post'); ?></span>
                </div>
            </div>
    ...
    function custom_font_callback($post) {
        ...
        echo '<span class="button ultp-font-variation-action">'.__('Add Variation', '').'</span>';
    }


    function metabox_save_data($post_id) {
        if (!isset($_POST['custom_font_nonce'])) { return; }
        if (!wp_verify_nonce( wp_unslash($_POST['custom_font_nonce']), 'font_meta_box')) { return; }

        $arr = array();
        if (isset($_POST['weight'])) {
            foreach ($_POST['weight'] as $i => $value) {
